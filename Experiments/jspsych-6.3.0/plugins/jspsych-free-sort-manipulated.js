...
jsPsych.plugins['free-sort'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      stim_locations: {
        ...
        pretty_name: 'Stimulus Locations',
        default: [],
        description: 'Locations we place each specified stimulus.'
      },
      stim_height: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var video_preload_blob = jsPsych.pluginAPI.getVideoBuffer(trial.vid_stimulus[0]);
    if(!video_preload_blob) {
      for(var i=0; i<trial.vid_stimulus.length; i++){
        var file_name = trial.vid_stimulus[i];
    ...
    for (let i = 0; i < trial.stimuli.length; i++) {
      ...
      if (trial.stim_starts_inside) {
      	console.log("ok");
      	if (trial.stim_locations.length != 0){
      		coords = {x: trial.stim_locations[i*2], y: trial.stim_locations[(i*2) + 1]};
      	} else{
